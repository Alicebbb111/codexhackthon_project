...
function registerUser(username, name, email, password, targetCareer = null) { return callApi("/auth/register", null, { method:"POST", body:JSON.stringify({username,name,email,password,target_career:targetCareer}) }); }
function saveDashboardResult(userId, result) { const s=result.categoryScores||{}; return callApi(`/dashboard/${userId}`,null,{method:"POST",body:JSON.stringify({technical_score_avg:s.technical,soft_score_avg:s.soft,readiness_score_avg:result.careerReadiness,recommended_career:result.career,market_match_percentage:result.careerMatch})}); }
